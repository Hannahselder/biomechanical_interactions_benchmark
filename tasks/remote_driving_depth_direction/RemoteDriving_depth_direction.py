...
class RemoteDriving_depth_direction(BaseTask):
  ...
  def __init__(self, model, data, end_effector, **kwargs):
    ...
    self._info = {"target_hit": False, "inside_target": False, "car_moving": False,
                  ...
                  "extratime_given": False, "joystick_touch": False}
    
    euler_angles = [-1.57, 0, 0]     
    quat = R.from_euler("xyz", euler_angles).as_quat()
    model.body("car").quat = quat
    model.joint("car").axis = [0, 1, 0]
    model.joint("target").axis = [1, 0, 0]
  ...
  def _update(self, model, data):
    ...
    geom1 = mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_GEOM, "thumb-stick-1")
    geom2 = mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_GEOM, "fingertip")
    for i in range(data.ncon):
        contact = data.contact[i]
        if (contact.geom1 == geom1 and contact.geom2 == geom2) or \
        (contact.geom1 == geom2 and contact.geom2 == geom1):
            joystick_touch=True
    self._info["joystick_touch"] = joystick_touch
