...
class Tracking_3D_target_area(BaseTask):

  def __init__(self, model, data, end_effector, shoulder, freq_curriculum, reward_function, weight=1, **kwargs):
    ...
    self._target_limits_x = np.array([-0.3, 0.3])
    self._target_limits_y = np.array([-0.3, 0.3])
    self._target_limits_z = np.array([-0.3, 0.3])

    # Update plane location
    model.geom("target-plane").size = np.array([(self._target_limits_x[1] - self._target_limits_x[0])/2,
    ...
    # Generate trajectory
    self.sin_x, self._sin_y, self._sin_z = self._generate_trajectory()
  ...
  def _update(self, model, data):
    ...
    self._info["target_position"] = self._target_position
  ...
  def _reset(self, model, data):
    ...
    self._sin_x, self._sin_y, self._sin_z = self._generate_trajectory()
  ...
  def _generate_trajectory(self):
    sin_x = self._generate_sine_wave(self._target_limits_x, num_components=5)
  ...
  def _update_target_location(self, model, data):
    self._target_position[0] = self._sin_x[self._steps]
