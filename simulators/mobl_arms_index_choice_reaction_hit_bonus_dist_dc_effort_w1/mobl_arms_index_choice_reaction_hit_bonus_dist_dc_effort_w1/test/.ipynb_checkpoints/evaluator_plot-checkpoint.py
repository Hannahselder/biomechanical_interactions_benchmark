...
def plot(i, name, forces, dists, ee_positions_x, ee_positions_y, ee_positions_z, target_positions_x, target_positions_y, target_positions_z):
    import matplotlib.pyplot as plt
    
    plt.plot(forces)
    plt.savefig(f'plots_dc/forces{i}.png')
    plt.clf()

    plt.plot(dists)
    plt.savefig(f'plots_dc/dists{i}.png')
    plt.clf()

    fig = plt.figure(figsize=(10, 8))
    ax = fig.add_subplot(111, projection='3d')
    
    ax.plot(ee_positions_x, ee_positions_y, ee_positions_z, color='b')
    ax.plot(target_positions_x, target_positions_y, target_positions_z,  color='r')
    
    for azim in [0,90,180,270]:
        ax.view_init(elev=0, azim=azim)
        plt.savefig(f"plots_dc/3D_plot_rotated_{azim}.png")
    plt.clf()
...
if __name__ == "__main__":
    ...
    for episode_idx in range(args.num_episodes):
        ...
        # Reset environment
        obs, info = simulator.reset(seed=100)
        terminated = False
        truncated = False
        reward = 0
        ...
        while not terminated and not truncated:
            ...
            if _policy_loaded:
                ...
                simulator.action_space.seed(100)
            ...
            #ee_positions_x.append(ee_position[0])
            #ee_positions_y.append(ee_position[1])
            #ee_positions_z.append(ee_position[2])

            #target_positions_x.append(target_position[0])
            #target_positions_y.append(target_position[1])
            #target_positions_z.append(target_position[2])
            reward += r
            ...
            if args.logging:
                ...
                state.update(info)
                state_logger.log(episode_idx, state)
        #plot(i,name,forces, dists, ee_positions_x, ee_positions_y, ee_positions_z, target_positions_x, target_positions_y, target_positions_z)
        i += 1
        # print(f"Episode {episode_idx}: {simulator.get_episode_statistics_str()}")
