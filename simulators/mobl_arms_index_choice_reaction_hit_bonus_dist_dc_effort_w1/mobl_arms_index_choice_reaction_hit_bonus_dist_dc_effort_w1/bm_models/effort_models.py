...
class JAC(BaseEffortModel):
  ...
  def cost(self, model, data):
    ...
    r_effort = self._r1*np.linalg.norm(data.ctrl ** 2)**2 
    r_jacc = self._r2*np.linalg.norm(data.qacc[self._bm_model.independent_dofs])**2 
...
class DC(BaseEffortModel):
  ...
  def cost(self, model, data):
    self._effort_cost = self._r1*np.linalg.norm(data.ctrl)**2 
    return self._effort_cost
...
class CTC(BaseEffortModel):
  ...
  def __init__(self, bm_model, r1=0.649, r2=0.0177, **kwargs):
    ...
    self.elbow_muscles = ["TRIlong", "TRIlat", "TRImed", "ANC", "SUP", "BRA", "PT", "PQ"]
  ...
  def cost(self, model, data):
    ...
    r_ctc = self._r2 * (np.abs(applied_shoulder_torque - self._previous_torque_shoulder) + np.abs(applied_elbow_torque - self._previous_torque_elbow))
