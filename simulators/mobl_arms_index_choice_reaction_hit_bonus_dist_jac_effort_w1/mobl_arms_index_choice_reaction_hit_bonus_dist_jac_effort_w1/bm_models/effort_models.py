...
class ArmMovementPaper(BaseEffortModel):
  ...
  def cost(self, model, data):
    ...
    shoulder_torque = data.ctrl[shoulder_id]  # Applied control torque
...
class JAC(BaseEffortModel):
  ...
  def __init__(self, bm_model, r1=0.0198, r2=6.67e-5, **kwargs):
    super().__init__(bm_model)
    self._r1 = r1
    self._r2 = r2
    self._effort_cost = None
  ...
  def cost(self, model, data):
    #print("r1", type(self._r1), self._r1, "effort", np.sum(data.ctrl ** 2)**2 , type(np.sum(data.ctrl ** 2)**2))  
    r_effort = self._r1*np.sum(data.ctrl ** 2)**2 
    r_jacc = self._r2*np.sum(data.qacc)**2 

    self._effort_cost = r_effort + r_jacc
...
class DC(BaseEffortModel):
  ...
  def __init__(self, bm_model, r1=0.1477, **kwargs):
    super().__init__(bm_model)
    self._r1 = r1
    self._effort_cost = None

  def cost(self, model, data):
    self._effort_cost = self._r1*np.sum(data.ctrl)**2 
    return self._effort_cost
...
class CTC(BaseEffortModel):
  ...
  def __init__(self, bm_model, r1=0.649, r2=0.0177, **kwargs):
    super().__init__(bm_model)
    self._r1 = r1
    self._r2 = r2
    self._effort_cost = None
  ...
  def cost(self, model, data):
    r_effort = self._r1*np.sum(data.ctrl)**2 
    ...
    r_ctc = self._r2 * (np.abs(shoulder_torque - self._previous_torque_shoulder) + np.abs(elbow_torque - self._previous_torque_elbow))
      
    self._previous_torque_shoulder = shoulder_torque
    self._previous_torque_elbow = elbow_torque

    self._effort_cost = r_effort + r_ctc
...
class Quadratic(BaseEffortModel):

  def __init__(self, bm_model, wv=1.9, wf=1, wr=1e-6, **kwargs):
    super().__init__(bm_model)
    self._wv = wv
    self._wf = wf
    self._wr = wr
  ...
  def cost(self, model, data):
    ...
    acceleration = self._wf * np.linalg.norm(data.qacc[self._bm_model.independent_dofs] ** 2)
    velocity = self._wv * np.linalg.norm(data.qvel[self._bm_model.independent_dofs] ** 2)
    effort = self._wr * np.linalg.norm(data.ctrl ** 2)

    self._effort_cost = acceleration + velocity + effort
