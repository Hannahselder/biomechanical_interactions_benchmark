...
class Tracking_rewards(BaseTask):

  def __init__(self, model, data, end_effector, shoulder, freq_curriculum, reward_function, **kwargs):
    super().__init__(model, data, **kwargs)

    bonus = kwargs.get('kwargs', {}).get('bonus', 8)
    dist_weight = kwargs.get('kwargs', {}).get('dist_weight', 1)
    print(dist_weight, bonus)
    ...
    # For logging
    self._info = {"terminated": False, "truncated": False, "termination": False, "inside_target": False, "distance_to_target": None, "target_position": None}
  ...
  def _update(self, model, data):
    ...
    target_pos = self._target_position + self._target_origin
    self._info["target_position"] = target_pos
    self._info["distance_to_target"] = dist
