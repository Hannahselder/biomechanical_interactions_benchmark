...
class Pointing_reach_envelope(BaseTask):
  ...
  def __init__(self, model, data, end_effector, shoulder, reward_function, weight=1, **kwargs):
    ...
    print("shoulder_pos", getattr(data, self._shoulder[0])(self._shoulder[1]).xpos.copy())
    ...
    self._target_reach_positions = self.get_reach_positions()
  ...
  def _spawn_target(self, model, data):
    ...
    for _ in range(10):
      new_position = 0.9*self.target_reach_positions[np.random.choice(len(self.target_reach_positions))]
    ...
    # Set location
    model.body("target").pos[:] = self._target_position #self._target_origin + 
  ...
  def get_reach_positions(self):

    base_dir = os.path.dirname(os.path.abspath(__file__))
    csv_path = os.path.join(base_dir, "uitb_distance_index3_test.csv")
    df = pd.read_csv(csv_path)
    self.target_reach_positions = df[[
        'end-effector_xpos_x',
        'end-effector_xpos_y',
        'end-effector_xpos_z'
    ]].values
