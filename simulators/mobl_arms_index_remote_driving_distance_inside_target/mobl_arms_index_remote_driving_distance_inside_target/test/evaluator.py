...
if __name__ == "__main__":
    ...
    parser.add_argument('--evaluate_number', type=int, default=None,
                        help='number of evaulation if results should be saved in according folder (default: None)')
    args = parser.parse_args()

    # Define directories
    checkpoint_dir = os.path.join(args.simulator_folder, 'checkpoints')

    if args.evaluate_number == None:
        evaluate_dir = os.path.join(args.simulator_folder, 'evaluate')
    else:
        evaluate_dir = os.path.join(args.simulator_folder, f'evaluate_{args.evaluate_number}')
    ...
    #torch.manual_seed(simulator.run_parameters["random_seed"])
    ...
    if args.record:
        ...
        #simulator._GUI_camera.write_video_set_path(os.path.join(evaluate_dir,sim_name + "_" + args.out_file))
        import pathlib
