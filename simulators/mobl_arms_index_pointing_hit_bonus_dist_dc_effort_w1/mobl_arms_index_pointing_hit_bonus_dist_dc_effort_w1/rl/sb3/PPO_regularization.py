  ...
  def __init__(self, simulator, checkpoint_path=None, wandb_id=None, info_keywords=()):
    ...
    if checkpoint_path is not None:
        ...
        self.model = PPO_sb3_customlogs.load(checkpoint_path, parallel_envs, verbose=1, #policy_kwargs=rl_config["policy_kwargs"], 
                                  ...
                                  learning_rate=rl_config["lr"], device=rl_config["device"], seed=run_parameters.get("random_seed", None), ent_coef=rl_config.get("ent_coef", 0))
    ...
    #print("set weights")
    parallel_envs.set_attr("action_net_weights", self.model.policy.state_dict()["action_net.weight"].cpu().numpy(), list(range(self.n_envs)))
    #print(parallel_envs.get_attr("action_net_weights", list(range(self.n_envs))))
    ...
    #model_path = os.path.join(simulator_folder, "initial_model_seed200.zip")
...
class PPO_sb3_customlogs(PPO_sb3):
   ...
   def collect_rollouts(
        ...
        while n_steps < n_rollout_steps:
            ...
            #print(env.get_attr("action_net_weights", list(range(self.n_envs))), "in collect_rollout")
            new_obs, rewards, dones, infos = env.step(clipped_actions)#Diese Zeile ruft simulator.__init__ auf
            #env.step_async(clipped_actions)
            #for remote, action in zip(env.remotes, clipped_actions):
