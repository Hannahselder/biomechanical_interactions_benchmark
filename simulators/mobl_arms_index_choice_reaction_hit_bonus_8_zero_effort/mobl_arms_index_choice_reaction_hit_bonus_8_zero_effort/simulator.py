...
def add_distance_sensors(simulation):
    """
    adds distance sensors to xml file
    """
    root = simulation.getroot()
    
    sensor_block = root.find(".//sensor")  # Falls <sensor> tiefer liegt, wird XPath verwendet
    
    distances = [
        {"geom1": "button-0", "geom2": "hand_2distph", "name": "button-0_distance", "cutoff": "100000"},
        {"geom1": "button-1", "geom2": "hand_2distph", "name": "button-1_distance", "cutoff": "100000"},
        {"geom1": "button-2", "geom2": "hand_2distph", "name": "button-2_distance", "cutoff": "100000"},
        {"geom1": "button-3", "geom2": "hand_2distph", "name": "button-3_distance", "cutoff": "100000"},
    ]
    
    # Distance-Elemente erstellen und hinzufügen
    for dist in distances:
        distance_element = ET.Element("distance", dist)
        sensor_block.append(distance_element)
    return simulation
...
class Simulator(gym.Env):
  ...
  def build(cls, config):
    ...
    if os.path.exists(simulation_file +".xml"):
      print("simulation_file already exists") 
    else:
      root = simulation.getroot()
      sensor_block = root.find(".//sensor")
      if sensor_block is not None:
        simulation = add_distance_sensors(simulation)
      with open(simulation_file +".xml", 'w') as file:
        simulation.write(file, encoding='unicode')
        print("simulation file is created")
  ...
  def _initialise(cls, config, simulator_folder, run_parameters):
    ...
    task_cls = cls.get_class("tasks", config["simulation"]["task"]["cls"])
    task_name = config["simulation"]["task"]["cls"]
    ...
    # Get simulation file
    simulation_file = os.path.join(simulator_folder, config["package_name"], "simulation")
    if task_name == "ChoiceReaction_distance_sensor_6_buttons":
        sim_dir = Path(__file__).resolve().parent
        xml_path = sim_dir / "simulation.xml"
        temp_path = sim_dir / "temp_simulation.xml"
        tree = ET.parse(xml_path)
        root = tree.getroot()
        sensor_block = root.find(".//sensor")  # Falls <sensor> tiefer liegt, wird XPath verwendet
    
        distances = [
            {"geom1": "button-4", "geom2": "hand_2distph", "name": "button-4_distance", "cutoff": "100000"},
            {"geom1": "button-5", "geom2": "hand_2distph", "name": "button-5_distance", "cutoff": "100000"},
        ]
        for dist in distances:
            distance_element = ET.Element("distance", dist)
            sensor_block.append(distance_element)
        ...
        touches = [
            {"site": "button-4", "name": "button-4"},
            {"site": "button-5", "name": "button-5"},
        ]
        for dist in touches:
            touch_element = ET.Element("touch", dist)
            sensor_block.append(touch_element)
        ...
        def create_button(name, pos, color):
            body = ET.Element("body", name=name, pos=pos, euler="0 -0.79 0")
            geom = ET.Element("geom", name=name, type="box", size="0.025 0.025 0.01", rgba=color)
            site = ET.Element("site", name=name, type="box", pos="0 0 0.01", size="0.02 0.02 0.01")
            body.extend([geom, site])
            return body

        worldbody.append(create_button("button-4", "0.482445 -0.38 0.943 ", "0.5 0 0.5 1.0"))
        worldbody.append(create_button("button-5", "0.392445 -0.38 0.843 ", "1.0 0.5 0 1.0"))
  ...
  def reset(self, seed=None):
    ...
    super().reset(seed=self._run_parameters.get("random_seed", None))
