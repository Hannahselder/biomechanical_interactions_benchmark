...
class ChoiceReaction_distance_sensor_mixed_button_positions(BaseTask):
  ...
  def __init__(self, model, data, end_effector, shoulder, reward_function, weight=1, **kwargs):
    ...
    self._buttons_positions = np.array([
        shoulder_pos + [0.41, -0.07, -0.15],
        shoulder_pos + [0.41,  0.07, -0.15],
        shoulder_pos + [0.5,  -0.07, -0.05],
        shoulder_pos + [0.5,   0.07, -0.05]
    ])
  ...
  def _choose_button(self, model, data):
    ...
    shoulder_pos = getattr(data, self._shoulder[0])(self._shoulder[1]).xpos.copy()
    permuted_positions = np.random.permutation(self._buttons_positions)

    #change button positions randomly
    for i, name in enumerate(self._buttons):
        model.body(name).pos[:] = permuted_positions[i]
