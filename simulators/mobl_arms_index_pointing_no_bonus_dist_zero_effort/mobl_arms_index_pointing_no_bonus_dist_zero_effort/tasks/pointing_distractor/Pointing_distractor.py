...
class Pointing_distractor(BaseTask):

  def __init__(self, model, data, end_effector, shoulder, reward_function, weight=1, **kwargs):
    ...
    self._reward_function = RewardFactory.create_reward_function(reward_function, k=weight)
  ...
  def _spawn_target(self, model, data):
    ...
    for _ in range(10):
      distractor_y = self._rng.uniform(*self._target_limits_y)
      distractor_z = self._rng.uniform(*self._target_limits_z)
      new_distractor_position = np.array([0, distractor_y, distractor_z])
      distance = np.linalg.norm(new_position - new_distractor_position)
      if distance > 2*self._target_radius + 0.001:
        break
    print("new_position", new_position, "new_distractor_position", new_distractor_position)
    distractor_position = new_distractor_position

    # Set location
    model.body("target").pos[:] = self._target_origin + self._target_position
    model.body("distractor").pos[:] = self._target_origin + new_distractor_position

    # Set target radius
    model.geom("target").size[0] = self._target_radius
    model.geom("distractor").size[0] = self._target_radius
