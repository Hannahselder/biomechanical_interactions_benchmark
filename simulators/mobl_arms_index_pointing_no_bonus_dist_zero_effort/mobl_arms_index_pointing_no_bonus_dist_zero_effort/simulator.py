...
class Simulator(gym.Env):
  ...
  def _initialise(cls, config, simulator_folder, run_parameters):
    ...
    if task_name == "Pointing_distractor":
        ...
        def create_distractor(name, pos, color):
            body = ET.Element("body", name=name, pos=pos)
            geom = ET.Element("geom", name=name, type="sphere", size="0.025", rgba=color)
            body.extend([geom])
            return body

        worldbody.append(create_distractor("distractor", "0.4 -0.41 0.928 ", "1 0 0 1"))
